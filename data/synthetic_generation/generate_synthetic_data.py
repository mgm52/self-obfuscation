...
data/synthetic_rating/rate_synthetic_data.py, which rates
each response for every concept to build the rated training datasets in
data/synthetic_rating/outputs/.
"""
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        help="Concepts (adjectives) to generate data for. Defaults to the 11 training concepts used in the paper.",
    )
    parser.add_argument(
        "--model-name",
    ...
    parser.add_argument(
        "--output-dir",
        default=None,
        help="Output directory (default: data/synthetic_generation/outputs/<timestamp>).",
    )
    args = parser.parse_args()

    time_at_start = datetime.now().strftime("%Y%m%d_%H%M%S")
    output_dir = args.output_dir or f"data/synthetic_generation/outputs/{time_at_start}"
