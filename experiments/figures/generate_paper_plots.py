...
def load_metric_results(
    ...
    for _, row in df.iterrows():
        ...
        # Value. Never default to 0.0 here: in this codebase a 0 TPR reads as
        # perfect probe evasion, so a missing column must fail loudly rather than
        # silently become the paper's headline result.
        value = None
        for c in guesses:
        ...
        if value is None:
            raise ValueError(
                f"No '{metric}' value column found in {csv_path} for concept '{concept}' "
                f"(condition '{condition}'). Tried: {guesses}. Available: {list(df.columns)}. "
                f"This summary.csv was produced without that metric -- regenerate it, or "
                f"select a threshold the file actually contains."
            )
...
def save_detailed_json(args, processed_results: List[ProcessedResult], output_folder):
    ...
    output_dict = {
        ...
        'command': 'python experiments/figures/generate_paper_plots.py',
        'arguments': vars(args),
...
def main():
    ...
    default_dir = 'outputs/figures/dummy_data/'
    # args (kept as in your original)
    arg_configs = [
        ('--train-probe-results', f'results/evaluation/organism_1layer_logistic/training_concepts/concept_probe_results.csv', 'Concept training probe results CSV'),
        ('--synth-harmful-results', f'results/evaluation/organism_1layer_logistic/harmful_synthetic/summary.csv', 'Synthetic harmful results summary CSV'),
        ('--dolus-deception', f'results/evaluation/organism_1layer_logistic/deception_doluschat/summary.csv', 'Deception (dolus) results summary CSV'),
        ('--apollorepe-deception', f'results/evaluation/organism_1layer_logistic/deception_repe/summary.csv', 'ApolloREPE deception results summary CSV'),
        ('--obfact-harmful', f'results/evaluation/organism_1layer_logistic/harmful_obfact/summary.csv', 'ObfAct harmful results summary CSV'),

        ('--base-synth-harmful-results', f'results/evaluation/base_model/harmful_synthetic/summary.csv', 'Base model synthetic harmful results summary CSV'),
        ('--base-dolus-deception', f'results/evaluation/base_model/deception_doluschat/summary.csv', 'Base model deception (dolus) results summary CSV'),
        ('--base-apollorepe-deception', f'results/evaluation/base_model/deception_repe/summary.csv', 'Base model ApolloREPE deception results summary CSV'),
        ('--base-obfact-harmful', f'results/evaluation/base_model/harmful_obfact/summary.csv', 'Base model ObfAct harmful results summary CSV'),
    ]
    ...
    parser.add_argument(
        '--output-folder',
        type=str,
        default=f"outputs/figures/plot_set_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}",
        help="Output folder"
    )
    # Cosine similarity pickle file arguments
    parser.add_argument('--apollorepe-cosine', type=str, default="results/evaluation/organism_1layer_logistic/deception_repe/cosine_similarity_raw_data_checkpoint_gemma_2_9b_final_layer.pkl")
    parser.add_argument('--deception-cosine', type=str, default="results/evaluation/organism_1layer_logistic/deception_doluschat/cosine_similarity_raw_data_checkpoint_gemma_2_9b_final_layer.pkl")
    parser.add_argument('--harmful-cosine', type=str, default="results/evaluation/organism_1layer_logistic/harmful_synthetic/cosine_similarity_raw_data_checkpoint_gemma_2_9b_final_layer.pkl")
    parser.add_argument('--obfact-cosine', type=str, default="results/evaluation/organism_1layer_logistic/harmful_obfact/cosine_similarity_raw_data_checkpoint_gemma_2_9b_final_layer.pkl")
    # Cosine similarity pickle file arguments
    parser.add_argument('--apollorepe-cosine-deception-ref', type=str, default="results/evaluation/organism_1layer_logistic/deception_repe/cosine_similarity_raw_data_checkpoint_gemma_ref_deception.pkl")
    parser.add_argument('--deception-cosine-deception-ref', type=str, default="results/evaluation/organism_1layer_logistic/deception_doluschat/cosine_similarity_raw_data_checkpoint_gemma_ref_deception.pkl")
    parser.add_argument('--harmful-cosine-harmful-ref', type=str, default="results/evaluation/organism_1layer_logistic/harmful_synthetic/cosine_similarity_raw_data_checkpoint_gemma_ref_harmful.pkl")
    parser.add_argument('--obfact-cosine-harmful-ref', type=str, default="results/evaluation/organism_1layer_logistic/harmful_obfact/cosine_similarity_raw_data_checkpoint_gemma_ref_harmful.pkl")

    # Base/checkpoint likelihood file arguments
    parser.add_argument('--dolus-base-likelihood', type=str, default='results/behavior_preservation/organism_1layer_logistic/deception_doluschat/base_model_likelihoods_gemma_2_9b_it_abliterated_ilya_20250908_004825.json')
    parser.add_argument('--repe-base-likelihood', type=str, default='results/behavior_preservation/organism_1layer_logistic/deception_repe/base_model_likelihoods_gemma_2_9b_it_abliterated_ilya_20250907_235501.json')
    parser.add_argument('--synth-harmful-base-likelihood', type=str, default='results/behavior_preservation/organism_1layer_logistic/harmful_synthetic/base_model_likelihoods_gemma_2_9b_it_abliterated_ilya_20250910_022304.json')
    parser.add_argument('--obfact-harmful-base-likelihood', type=str, default='results/behavior_preservation/organism_1layer_logistic/harmful_obfact/base_model_likelihoods_gemma_2_9b_it_abliterated_ilya_20250908_012626.json')

    parser.add_argument('--dolus-checkpoint-likelihood', type=str, default='results/behavior_preservation/organism_1layer_logistic/deception_doluschat/checkpoint_model_likelihoods.json')
    parser.add_argument('--repe-checkpoint-likelihood', type=str, default='results/behavior_preservation/organism_1layer_logistic/deception_repe/checkpoint_model_likelihoods.json')
    parser.add_argument('--synth-harmful-checkpoint-likelihood', type=str, default='results/behavior_preservation/organism_1layer_logistic/harmful_synthetic/checkpoint_model_likelihoods.json')
    parser.add_argument('--obfact-harmful-checkpoint-likelihood', type=str, default='results/behavior_preservation/organism_1layer_logistic/harmful_obfact/checkpoint_model_likelihoods.json')
    ...
    parser.add_argument('--synth-harmful-mlp', type=str, default="results/evaluation/organism_mlp/harmful_synthetic/summary.csv")
    parser.add_argument('--obfact-harmful-mlp', type=str, default="results/evaluation/organism_mlp/harmful_obfact/summary.csv")
    parser.add_argument('--dolus-deception-mlp', type=str, default="results/evaluation/organism_mlp/deception_doluschat/summary.csv")
    parser.add_argument('--apollorepe-deception-mlp', type=str, default="results/evaluation/organism_mlp/deception_repe/summary.csv")

    parser.add_argument('--synth-harmful-attention', type=str, default="results/evaluation/organism_attention/harmful_synthetic/summary.csv")
    parser.add_argument('--obfact-harmful-attention', type=str, default="results/evaluation/organism_attention/harmful_obfact/summary.csv")
    parser.add_argument('--dolus-deception-attention', type=str, default="results/evaluation/organism_attention/deception_doluschat/summary.csv")
    parser.add_argument('--apollorepe-deception-attention', type=str, default="results/evaluation/organism_attention/deception_repe/summary.csv")

    parser.add_argument('--synth-harmful-4probe', type=str, default="results/evaluation/organism_4layer_logistic/harmful_synthetic/summary.csv")
    parser.add_argument('--obfact-harmful-4probe', type=str, default="results/evaluation/organism_4layer_logistic/harmful_obfact/summary.csv")
    parser.add_argument('--dolus-deception-4probe', type=str, default="results/evaluation/organism_4layer_logistic/deception_doluschat/summary.csv")
    parser.add_argument('--apollorepe-deception-4probe', type=str, default="results/evaluation/organism_4layer_logistic/deception_repe/summary.csv")

    parser.add_argument('--synth-harmful-8probe', type=str, default="results/evaluation/organism_8layer_logistic/harmful_synthetic/summary.csv")
    parser.add_argument('--obfact-harmful-8probe', type=str, default="results/evaluation/organism_8layer_logistic/harmful_obfact/summary.csv")
    parser.add_argument('--dolus-deception-8probe', type=str, default="results/evaluation/organism_8layer_logistic/deception_doluschat/summary.csv")
    parser.add_argument('--apollorepe-deception-8probe', type=str, default="results/evaluation/organism_8layer_logistic/deception_repe/summary.csv")

    # LMEval result file arguments
    parser.add_argument('--lmeval-base', type=str, default=f'results/capability_evals/base_model/results_2025-09-09T01-41-07.239927_normal.json')
    parser.add_argument('--lmeval-checkpoint', type=str, default='results/capability_evals/organism_1layer_logistic/results_2025-09-10T19-29-42.471322_normal.json')
    parser.add_argument('--lmeval-checkpoint-harmful', type=str, default='results/capability_evals/organism_1layer_logistic/results_2025-09-10T18-50-26.444075_harm-focused.json')
    parser.add_argument('--lmeval-checkpoint-untruthful', type=str, default='results/capability_evals/organism_1layer_logistic/results_2025-09-10T18-42-45.839583_lying.json')
